...
export default function JobTypes() {
  // addition of logic for filtering the JobTypes
  const [jobTitle, setJobTitle] = useState(types[0]);
  const [filteredJobTypes, setFilteredJobTypes] = useState([]);

  //function to filter the JobTypes
  const filterJobs = (type) => {
    return allJobs.filter((job) => job.jobType === type);
  };

  //update filtered Job when jobTitle is changed
  useEffect(() => {
    setFilteredJobTypes(filterJobs(jobTitle));
  }, [jobTitle]);
  // end of logic
